...
class AcademicInfo
{
    ...
    public function __invoke(DataTableBuilder $table)
    {
        $table->relation('academic', function (DataTableBuilder $table){
            $table
                ->addAutocomplete('autocomplete/faculty', trans('admin::app.faculty'), 'academic.faculty_id', "academic.faculty.transName.text", "academic.faculty.lang_name.$this->lang.text", 'req required none', '', '', true, false, false, false)
                ...
                ->addInputText(trans('admin::app.gpa'), 'academic.gpa', 'academic.gpa','req required')
            ;
        });
    }
}